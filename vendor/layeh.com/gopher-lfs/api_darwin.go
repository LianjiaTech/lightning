...
func attributesFill(tbl *lua.LTable, stat os.FileInfo) {
	...
	tbl.RawSetH(lua.LString("access"), lua.LNumber(sys.Atimespec.Sec))
	tbl.RawSetH(lua.LString("modification"), lua.LNumber(sys.Mtimespec.Sec))
	tbl.RawSetH(lua.LString("change"), lua.LNumber(sys.Ctimespec.Sec))
